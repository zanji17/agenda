            ...
            <div class="panel-heading"><h3 class="panel-title"><span class="glyphicon glyphicon-glass" aria-hidden="true"></span>&nbsp;Aniversariantes do Mês</h3></div>
                <div class="panel-body">
                    ...
                    <?php
                        ...
                        $hoje=date('Y-m-d');
                        $atual=explode("-",$hoje);
                        $sql2 = mysqli_query($conn, "SELECT * FROM pessoa");
                        while($pessoa = mysqli_fetch_array($sql2)){
                            $aniversario=explode("-",$pessoa['datanascimento']);
                            if($atual[1]==$aniversario[1]){
                                $id = $pessoa['id'];
                                $data = date_create($pessoa['datanascimento']);
                                echo "<td><a href='verpessoa.php?id=$id'>".$pessoa['nome']."</a>";
